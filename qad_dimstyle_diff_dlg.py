...
#######################################################################################
# Class that manages the graphical interface of the dimensioning style comparison function
class QadDIMSTYLE_DIFF_Dialog(QDialog, QObject, qad_dimstyle_diff_ui.Ui_DimStyle_Diff_Dialog):
   ...
   def showDiffProps(self, dimStyle1, dimStyle2):
      ...
      self.count = 0
      prop1Items = dimStyle1.getPropList().items() # name list with list [description, value]
      props2Dict = dimStyle2.getPropList() # name dictionary with list [description, value]
   ...
   def resizeEvent(self, event):
      ...
      if event.oldSize().width() == -1: # there was no previous dimension
         return
