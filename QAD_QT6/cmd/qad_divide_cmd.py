...
class QadDIVIDECommandClassStepEnum():
   ...
   ASK_SEGMENT_NUMBER = 3 # requires the number of segments
...
class QadDIVIDECommandClass(QadCommandClass):
   ...
   def run(self, msgMapTool = False, msg = None):
      ...
      elif self.step == QadDIVIDECommandClassStepEnum.ASK_FOR_ALIGNMENT: # after waiting for a keyword the command is restarted
      ...
      # =========================================================================
      # RESPONSE TO THE REQUEST FOR THE NUMBER OF SEGMENTS (from step = ASK_FOR_ALIGNMENT)
      # =========================================================================
      elif self.step == QadDIVIDECommandClassStepEnum.ASK_SEGMENT_NUMBER: # after waiting for an integer the command is restarted
         if msgMapTool == True: # the point comes from a graphic selection
            if self.getPointMapTool().rightButton == True: # if used with the right mouse button
               return False
            else:
