...
/***************************************************************************
 ...
 class to manage the text window
...
class QadInputTypeEnum():
   NONE     = 0    # none
   COMMAND  = 1    # name of a command
   POINT2D  = 2    # point
   POINT3D  = 4    # point
   KEYWORDS = 8    # a keyword
   STRING   = 16   # a string
   INT      = 32   # an integer
   LONG     = 64   # an integer
   FLOAT    = 128  # a real number
   BOOL     = 256  # a boolean value
   ANGLE    = 512  # a real value in degrees
...
class QadInputModeEnum():
   ...
   NOT_ZERO     = 2   # zero value not allowed
   NOT_NEGATIVE = 4   # negative value not allowed
   NOT_POSITIVE = 8   # positive value not allowed
...
class QadTextWindow(QDockWidget, Ui_QadTextWindow):
   ...
   def readDockWidgetSettings(self):
      ...
      defaultDockWidgetArea = getattr(Qt.DockWidgetArea.BottomDockWidgetArea, "value", Qt.DockWidgetArea.BottomDockWidgetArea)
      dockWidgetAreaValue = s.value("qad/text_window_area", defaultDockWidgetArea, type=int,)
      try:
         dockWidgetArea = Qt.DockWidgetArea(dockWidgetAreaValue)
      except ValueError:
   ...
   def showInputMsg(self, inputMsg = None, inputType = QadInputTypeEnum.COMMAND, \
      ...
      # the default value of a function parameter cannot be a translation
      # because lupdate.exe cannot interpret it
      ...
      if inputSearchOptions & QadINPUTSEARCHOPTIONSEnum.ON and inputSearchOptions & QadINPUTSEARCHOPTIONSEnum.DISPLAY_LIST:
         if mode == True:
            ...
            if self.isFloating():
               ...
               rect = qad_utils.getScreenGeometry(self.edit, ptUp)
               ...
               rect = qad_utils.getScreenGeometry(self.edit, pt)
...
class QadChronologyEdit(QTextEdit):
   ...
   # set_Colors
   # ============================================================================
   def set_Colors(self, foregroundColor = QColor(Qt.GlobalColor.black), backGroundColor = QColor(Qt.GlobalColor.lightGray)):
   ...
   def insertText(self, txt):
      ...
      for line in txt.split('\n'):
         if len(line) > 0: # to avoid one more empty line
            cursor.movePosition(QTextCursor.End, QTextCursor.MoveAnchor) # end of document
...
class QadEdit(QTextEdit):
   ...
   def __init__(self, parent, chronologyEdit):
      ...
      # string containing keywords separated by "/".
      # the string may contain the special character "_" to separate keywords
      # in the local language from those in English (e.g. "Yes/No/Other option_Yes/No/Other option")
      self.englishKeyWords = [] # parole chiave in inglese
      self.cmdOptionPosList = [] # list of options positions of the current command
      self.currentCmdOptionPos = None

      self.upperKeyWordForegroundColor = QColor(Qt.GlobalColor.blue)
      self.keyWordBackGroundColor = QColor(210, 210, 210)
      self.keyWordHighlightBackGroundColor = QColor(Qt.GlobalColor.gray)
   ...
   # set_Colors
   # ============================================================================
   def set_Colors(self, foregroundColor = QColor(Qt.GlobalColor.black), backGroundColor = QColor(Qt.GlobalColor.white)):
      f = QColor(foregroundColor)
      b = QColor(backGroundColor)
      rgbStrForeColor = "rgb({0},{1},{2})"
      rgbStrForeColor = rgbStrForeColor.format(str(f.red()), str(f.green()), str(f.blue()))
   ...
   # set_keyWordColors
   # ============================================================================
   def set_keyWordColors(self, backGroundColor = QColor(210, 210, 210), upperKeyWord_ForegroundColor = QColor(Qt.GlobalColor.blue), \
                         highlightKeyWord_BackGroundColor = QColor(Qt.GlobalColor.gray)):
   ...
   def setFormat(self, start, count, fmt): # 1-indexed
      ...
      cursor.movePosition(QTextCursor.Start, QTextCursor.MoveAnchor) # beginning of document
   ...
   def highlightKeyWords(self):
      ...
      i = txt.find("[")
      final = txt.rfind("]")
      if i >= 0 and final > i: # if there are options
         i = i + 1
         pos = lastBlock.position() + i
         while i < final:
            if txt[i] != "/":
               # if there is a current option it must be highlighted differently
   ...
   def showMsg(self, msg, displayPromptAfterMsg = False, append = True):
      if len(msg) > 0:
         ...
         if self.inputType & QadInputTypeEnum.KEYWORDS:
            ...
            self.setCmdOptionPosList() # I initialize the list of keyWords positions
            self.highlightKeyWords()
         else:
            self.textCursor().block().setUserState(QadEdit.PROMPT)
            del self.cmdOptionPosList[:] # I empty the list of keyWords positions

      if displayPromptAfterMsg:
         self.displayPrompt() # repeats the prompt


   def showErr(self, err):
      self.showMsg(err, True) # repeats the prompt
      # if there is a map tool with dynamic input enabled
   ...
   def showNextCmd(self):
      ...
      if self.historyIndex < cmdsHistoryLen and cmdsHistoryLen > 0:
         ...
         if self.historyIndex < cmdsHistoryLen:
            ...
            self.showMsg(cmdsHistory[self.historyIndex], False, False) # replaces the text after the prompt
   ...
   def setCmdOptionPosList(self):
      del self.cmdOptionPosList[:] # I empty the list
      lenKeyWords = len(self.keyWords)
      if lenKeyWords == 0 or len(self.currentPrompt) == 0:
         return
      # the options are enclosed in square brackets and separated from each other by /
      ...
      if self.textCursor().position() >= self.document().lastBlock().position():
         if event.button() == Qt.LeftButton:
            ...
            if cmdOptionPos is not None:
               ...
               # this is to avoid that e.g. the "End" option is confused with osnap "end"
   ...
   def keyPressEvent(self, e):
      if self.parentWidget().plugin.shortCutManagement(e): # if a shortcut key sequence has been handled
         return
   ...
   def showEvaluateMsg(self, msg = None, append = True):
      """show and evaluate the message msg if different from None otherwise use the current message"""
      if msg is not None:
         self.showMsg(msg, False, append)
      self.entered()

   def getCurrMsg(self):
      """returns the message already present in the text window"""
      cursor = self.textCursor()
   ...
   def evaluate(self, cmd):
      ...
      if self.inputType & QadInputTypeEnum.COMMAND:
         if cmd == "":
            cmd = unicode(self.showLastCmd()) # I repeat last command
            if cmd == "":
               return
         ...
         if self.parentWidget().isValidCommand(cmd) or self.parentWidget().isValidEnvVariable(cmd):
            ...
            self.showErr(msg.format(cmd.encode('utf-8','ignore').decode('utf-8'))) # repeats the prompt
         return
      ...
      if self.inputType & QadInputTypeEnum.POINT2D:
         ...
         # I evaluate the case of "midpoint between 2 points"
      ...
      # 3D point
      #------------------------------------------------------------------------------
      if self.inputType & QadInputTypeEnum.POINT3D: # point
         pass
      ...
      if self.inputType & QadInputTypeEnum.INT:
         ...
         if num is not None:
            if num == 0 and (self.inputMode & QadInputModeEnum.NOT_ZERO): # not allowed value = 0
               num = None
            elif num < 0 and (self.inputMode & QadInputModeEnum.NOT_NEGATIVE): # not allowed value < 0
               num = None
            elif num > 0 and (self.inputMode & QadInputModeEnum.NOT_POSITIVE): # not allowed value > 0
               num = None
   ...
   def getOptimalHeight(self):
      ...
      pixelsWidth = fm.horizontalAdvance(QadMsg.translate("QAD", "Command: "))
      pixelsHeight = fm.height()
      # + 8 because QTextEdit has a vertical offset above and below the text
...
class QadCmdSuggestWindow(QWidget, Ui_QadCmdSuggestWindow, object):
   ...
   def __init__(self, parent, editWidget, infoCmds, infoVars):
      ...
      self.infoCmds = infoCmds[:] # I copy the command list
      self.infoVars = infoVars[:] # I copy the environment variable list
      self.filter = ""
   ...
   def getFilteredInfoList(self, infoList):
      inputSearchOptions = QadVariables.get(QadMsg.translate("Environment variables", "INPUTSEARCHOPTIONS"))
      # inputSearcOptions & QadINPUTSEARCHOPTIONSEnum.ON = Turns on all automated keyboard features when typing at the Command prompt
      # inputSearcOptions & QadINPUTSEARCHOPTIONSEnum.DISPLAY_ICON = Displays the icon of the command or system variable, if available.
      ...
      if len(upperFilter) > 0:
         if filter == "*": # if it begins with * it means all commands in the local language
            # list composed of elements with:
            # <command local name>, <command English name>, <icon>, <notes>
            ...
            if upperFilter[0] == "_": # versione inglese
               ...
               # <command local name>, <command English name>, <icon>, <notes>
               for info in infoList:
                   # if "starts with" or if "quite similar"
   ...
   def setFilter(self, filter = ""):
      ...
      inputSearchOptions = QadVariables.get(QadMsg.translate("Environment variables", "INPUTSEARCHOPTIONS"))
      # inputSearcOptions & QadINPUTSEARCHOPTIONSEnum.ON = Turns on all automated keyboard features when typing at the Command prompt
      # inputSearcOptions & QadINPUTSEARCHOPTIONSEnum.EXCLUDE_SYS_VAR = Excludes the display of system variables
      ...
      if l > 0:
         ...
         # I select the first one that begins with filter
   ...
   def getDataHeight(self):
      ...
      OffSet = 4 # a little more space to also show the command icon
   ...
   def showMsg(self, cmd):
      # I replace the text with the new command and return the cursor to the previous position
...
class QadCmdSuggestListView(QListView):
   ...
   def set(self, filteredCmdNames):
      # list composed of elements with <command name>, <icon>, <notes>
      self.model.clear()
